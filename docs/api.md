...
| data.ITEM_TYPE_HANDLERS            | `undefined/Object`                                                                                                                                                                              | An object that determines additional behavior for certain item types, such as containers, see [Item Piles: D&D 5e](https://github.com/fantasycalendar/FoundryVTT-ItemPilesDnD5e/blob/master/dist/module.js#L58-L77)                                                                                                                                                                                                                                                                               |
| data.ITEM_COST_TRANSFORMER         | `undefined/Function`                                                                                                                                                                            | An optional function that gets run when fetching the price of an item - expects the return value to be a number that is the total value in the primary currency                                                                                                                                                                                                                                                                                                                                   |
| data.ITEM_PREVIEW_PERMISSION_LEVEL | `undefined/Number`                                                                                                                                                                              | An optional number that configures the ownership level of the item window that is shown when players click on items in item piles. In some systems, the description is only visible when the level is set to "Observer" (2), while the default "Limited" (1) is fine for most systems.                                                                                                                                                                                                            |
