...
<div>
    <nav class="tabs" data-group="primary">
        {#each tabs.filter(tab => tab.visible) as tab, index (tab.value)}
            <a on:click={() => { activeTab = tab.value}} class="item flexrow" class:active={activeTab === tab.value} data-tab="rest">
                {#if tab.icon} <i class="icon {tab.icon}"></i> {/if}
                {localize(tab.label)}
                {#if tab.highlight} <div class="blob"><i class="fas fa-exclamation"></i></div> {/if}
            </a>
        {/each}
    </nav>
</div>
