<script>
    ...
    const attributesStore = currencyStore.attributes;
    ...
    function add() {
        ...
        console.log(get(currencyStore.primary))
    ...
    {#each $attributesStore as { id, primary, name, exchange, path, img }, index (index)}
        ...
        >
            <td class="small">
                ...
                <input type="radio" required name="primaryCurrency" checked={primary} on:change={() => { currencyStore.setPrimary(index) }}/>
            </td>
            ...
            <td class="small"><button type="button" on:click={attributeRemove(index)}><i class="fas fa-times"></i></button></td>
        </tr>
    {/each}
</table>
