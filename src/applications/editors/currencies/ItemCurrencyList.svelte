        ...
        {#each $itemStore as { primary, name, exchange, img }, index (index)}
            ...
            >
                <td class="small">
                    ...
                    <input type="radio" required name="primaryCurrency" checked={primary} on:change={() => { currencyStore.setPrimary(index, true) }}/>
                </td>
                ...
                <td class="small"><button type="button" on:click={itemRemove(index)}><i class="fas fa-times"></i></button></td>
            </tr>
        {/each}
    </table>
