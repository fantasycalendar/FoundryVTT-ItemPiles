<script>
   ...
   async function updateSettings() {
      ...
      if(!application.options.resolve){
         application.submit(itemFilters);
      }
      application.close();
   }
...
<form bind:this={form} on:submit|preventDefault={updateSettings} autocomplete=off>
   ...
   <p>{localize("ITEM-PILES.FilterEditor.Explanation")}</p>

   <table>
      <tr>
         <th>{localize("ITEM-PILES.FilterEditor.Path")}</th>
         <th>{localize("ITEM-PILES.FilterEditor.Filters")}</th>
      ...
      {#each itemFilters as { path, filters }, index (index)}
         <tr>
            ...
            <td class="custom-small"><button type="button" on:click={remove(index)}><i class="fas fa-times"></i></button></td>
         </tr>
      {/each}
   </table>
