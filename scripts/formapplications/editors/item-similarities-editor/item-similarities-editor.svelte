<script>
   ...
   async function updateSettings() {
      application.submit(itemSimilarities);
      application.close();
   }
...
<form bind:this={form} on:submit|preventDefault={updateSettings} autocomplete=off>
   <p>{localize("ITEM-PILES.SimilaritiesEditor.Explanation_P1")}</p>
   <p>{localize("ITEM-PILES.SimilaritiesEditor.Explanation_P2")}</p>

   <table>
      <tr>
         <th>{localize("ITEM-PILES.SimilaritiesEditor.Path")}</th>
